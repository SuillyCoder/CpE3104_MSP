...
DATA SEGMENT
   ...
   PORTA1 EQU 0F0H  ;ADC CONTROL LINES (USER SET TEMP)
   ...
   PORTC2 EQU 0FCH  ;ADC TEMPERATURE OUTPUT FED AS INPUT (USER SET TEMP)
   ...
   PORTA4 EQU 0E0H  ;ADC CONTROL LINES (OUTSIDE TEMP)
   PORTB4 EQU 0E2H  ;SYSTEM POWER SIGNAL
   PORTC4 EQU 0E4H  ;ADC TEMPERATURE OUTPUT FED AS INPUT (OUTSIDE TEMP)
   CMD_REG_4 EQU 0E6H ; COMMAND REGISTER FOR PPI3
   ...
   ;==================OUTSIDE TEMPERATURE ASSETS ====================;
   OUTSIDE_TEMP_VALUE DB 0    ; Stores the outside temperature reading
   TEMP_CHANGE_VALUE DB 0     ; Stores the user-set temperature (from first ADC)
   PREV_COMPRESSOR_STATE DB 0 ; Track previous compressor state to avoid flickering
   ...
   ; Speed level mapping
   SPEED_75_RPM   EQU 5   ; low but above stall
   SPEED_100_RPM  EQU 7    ; low-medium
   SPEED_150_RPM  EQU 10    ; medium
   SPEED_200_RPM  EQU 12   ; medium-high
   SPEED_250_RPM  EQU 14   ; high
   SPEED_300_RPM  EQU 15   ; max
...
START:
   ...
   CALL READ_ADC_AND_COMPARE      ; Read user-set temp
   CALL READ_OUTSIDE_TEMP          ; Read outside temp
   CALL UPDATE_COMPRESSOR_STATE    ; Set initial compressor state
...
INPUT_LOOP:
 ; Always check power button, even during shutdown
...
NORMAL_OPERATION:
   ...
   CALL INPUT_CHECK
    ...
    ; Step 1.5: Read outside temperature  
   CALL READ_OUTSIDE_TEMP          
   
   ; Step 1.6: Update compressor based on temperature comparison 
   CALL UPDATE_COMPRESSOR_STATE        
...
SKIP_BIT:
    ...
    MOV [TEMP_CHANGE_VALUE], CL  ; Store user-set temperature
...
; ================== READ OUTSIDE TEMPERATURE FROM PPI4 =====================

;NOTE: The second LM35 sensor (the one beside the compressor) reads the outside temperature
; This is to simulate how the compressor would work based on the readings of the outside temperature...
;...and configured set temperature from the user. 

;IF
;Outside Temp > User-Set Temperature = Compressor ON

;ELSE IF 
;Outside Temp = User-Set Temperature = Compressor OFF

;ELSE
;Outside Temp < User-Set Temperature = Compressor OFF


READ_OUTSIDE_TEMP PROC
   PUSH AX
   PUSH BX
   PUSH CX
   PUSH DX
   
   ; Step 1: Select ADC channel (IN0 = 000 for outside LM35)
   MOV DX, PORTA4
   MOV AL, 00000000B
   OUT DX, AL
   CALL DELAY_ADC
   ...
   ; Step 6: Read the data from PORTC4
   MOV DX, PORTC4
   IN AL, DX
   MOV CL, AL            ; Save ADC value in CL (reversed bits)
...
REVERSE_OUTSIDE_LOOP:
   SHL BL, 1             ; Shift result left
   SHR CL, 1             ; Shift input right, LSB to Carry
   JNC SKIP_OUTSIDE_BIT  ; If carry=0, skip
   OR BL, 1              ; If carry=1, set lowest bit
SKIP_OUTSIDE_BIT:
   DEC CH
   JNZ REVERSE_OUTSIDE_LOOP

   MOV CL, BL            ; CL now has normal binary value
   MOV [OUTSIDE_TEMP_VALUE], CL  ; Store it
...
UPDATE_MOTOR_SPEED PROC
   ...
   CMP [FAN_MODE], 1
   JE CHECK_FAN_SUBMODE
   CMP [FAN_MODE], 2
   JE CHECK_COOL_SUBMODE
   JMP APPLY_SPEED          ; This should never execute now
...
CHECK_FAN_SUBMODE:
   ...
   CMP [SUBMODE], 2
   JE FAN_TURBO_SPEED
   MOV BL, SPEED_200_RPM
   JMP APPLY_SPEED
   
FAN_ECO_SPEED:
   MOV BL, SPEED_100_RPM
   JMP APPLY_SPEED
   
FAN_TURBO_SPEED:
   MOV BL, SPEED_300_RPM
   JMP APPLY_SPEED
...
COOL_TURBO_SPEED:
   MOV BL, SPEED_250_RPM
   
APPLY_SPEED:
   CALL ENABLE_FAN_MOTOR
   CALL SET_FAN_MOTOR_SPEED
...
; ================== COMPRESSOR CONTROL BASED ON TEMPERATURE =====================

UPDATE_COMPRESSOR_STATE PROC
   PUSH AX
   PUSH BX
   PUSH DX
   
   ; Check if system is powered off or shutdown
   CMP [SYSTEM_POWERED_ON], 0
   JE FORCE_COMPRESSOR_OFF
   CMP [SYSTEM_SHUTDOWN], 1
   JE FORCE_COMPRESSOR_OFF
   
   ; Read both temperature values
   MOV AL, [OUTSIDE_TEMP_VALUE]    ; Outside temperature
   CMP AL, 0
   JE COMPRESSOR_EXIT           ; No reading yet, skip
   
   MOV BL, [TEMP_CHANGE_VALUE]     ; User-set temperature
   CMP BL, 0
   JE COMPRESSOR_EXIT           ; No user temp set, skip
   
   ; Compare: OUTSIDE_TEMP vs TEMP_CHANGE
   CMP AL, BL
   JG TURN_COMPRESSOR_ON    ; If OUTSIDE > TEMP_CHANGE, turn ON
   JE TURN_COMPRESSOR_OFF   ; If OUTSIDE = TEMP_CHANGE, turn OFF
   JL TURN_COMPRESSOR_OFF   ; If OUTSIDE < TEMP_CHANGE, turn OFF
   
TURN_COMPRESSOR_ON:
   ; Check if already ON to avoid unnecessary writes
   CMP [COMPRESSOR_STATE], 1
   JE COMPRESSOR_EXIT
   
   ; Turn ON compressor (set bit 0 of PORTA3)
   MOV DX, PORTA3
   IN AL, DX
   OR AL, 01H              ; Set bit 0
   OUT DX, AL
...
TURN_COMPRESSOR_OFF:
   ; Check if already OFF to avoid unnecessary writes
   CMP [COMPRESSOR_STATE], 0
   JE COMPRESSOR_EXIT
   
   ; Turn OFF compressor (clear bit 0 of PORTA3)
   MOV DX, PORTA3
   IN AL, DX
...
FORCE_COMPRESSOR_OFF:
   ; Force compressor OFF during shutdown/power off
   MOV DX, PORTA3
   IN AL, DX
   AND AL, 11111110B
   OUT DX, AL
...
TIMER_EXPIRED_HANDLER:
   ...
   MOV [SYSTEM_SHUTDOWN], 1   ; *** SET SHUTDOWN FLAG ***
...
CHECK_DISPLAY_TOGGLE PROC
   ...
   JNE NO_TOGGLE
    ...
    ; Check if we're recovering from shutdown
   CMP [SYSTEM_SHUTDOWN], 1
   JE RECOVER_FROM_SHUTDOWN
   
   ; Normal toggle power state
   ...
   CMP AL, 0
   JE TURN_SYSTEM_OFF
   JMP TURN_SYSTEM_ON_NORMAL
   
RECOVER_FROM_SHUTDOWN:
   ; Coming out of shutdown, force system ON
   MOV [SYSTEM_SHUTDOWN], 0
   MOV [SYSTEM_POWERED_ON], 1
   ...
   ; Clear shutdown flag when system is powered back on
   MOV [SYSTEM_SHUTDOWN], 0   ; *** ALLOW SYSTEM TO OPERATE NORMALLY ***
...
TURN_SYSTEM_ON_NORMAL:
   ; ===== TURN SYSTEM ON =====
   MOV AL, 0CH        ; Display ON, Cursor OFF
   CALL INST_CTRL
   MOV [LCD_DISPLAY_STATE], 1
   
   ; Restore a default fan mode so motor runs immediately
   MOV [FAN_MODE], 2          ; Default COOL mode (same as at START)
   MOV [SUBMODE], 0
   CALL UPDATE_MOTOR_SPEED    ; Recompute BL and re-enable fan outputs
